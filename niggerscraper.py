# THIS PART OF VANISH IS GRABBED FROM HELIUM! (only github edition lol)
...
def scraper(guildid, channelid):
    ...
    try:
        token = random.choice(tokensinguild)
         ...
         print(f"{datetime.datetime.now().strftime(f'{Fore.LIGHTBLACK_EX}%H:%M:%S')}{Fore.RESET}  {Fore.CYAN}FAILED   {Fore.RESET}No tokens in {Fore.CYAN}Guild{Fore.RESET}")
         exit(0)
...
def selfscraper(token, guildid, channelid):
    ...
    try:
        ...
        if rr.status_code == 200:
            print(f"  {datetime.datetime.now().strftime(f'{r}[{lc}%H:%M:%S')}{r}]  {r}[{lc}SCRAPPER{r}] {lc}»{r} {r}Scrapping..")
    except:
        print(f"  {datetime.datetime.now().strftime(f'{r}[{lc}%H:%M:%S')}{r}]  {r}[{lc}SCRAPPER{r}] {lc}»{r} {r}Join first {lc}Token{r} to {lc}Victim Server{r}.. ok?")
        exit(0)
    members = scrape(token, f"{guildid}", f"{channelid}")
    for member in members:
        print(f"  {datetime.datetime.now().strftime(f'{r}[{lc}%H:%M:%S')}{r}]  {r}[{lc}SCRAPPER{r}] {lc}»{r} {r}Scrapped {lc}{member}{r} from guild {r}{lc}{guildid}{r}")
